...
| テスト | ✅ **1228件パス**（45ファイル。内訳：コア 498件＋Electron 730件） |
...
| 8 | 孤立修正の再接続UI / 確認画面：マーカー — ★次に着手 | ⏳ 未着手 |
| 9 | 書き出し画面 | ⏳ 未着手 |
| 10 | AI設定（ローカルモード配線 → GeminiProvider） | ⏳ 未着手 |

★**カメラ切替（Step 7）だけが FCP7 XML を書き換えます。** 字幕はSRT、ショート候補はCSVにしか出ませんが、カメラ修正はPremiereプロジェクトの映像トラック構成そのものを変えます。Premiere実機検証を行う際は、カメラ切替を修正した状態でも確認してください。

3画面（字幕・ショート・カメラ）が出そろい共通項が確定しました。次の画面に着手する前に CURRENT-STATE.md の「10. 今後のリファクタリング候補」を確認してください。

> 実装の進捗・詳細・未検証事項は [CURRENT-STATE.md](./CURRENT-STATE.md) が唯一の正です。
> ⚠️ 本ファイルの「Phase 1/2/3」と「Step 1〜10」は**別の区分**です。
