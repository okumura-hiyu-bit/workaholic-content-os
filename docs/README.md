...
| テスト | ✅ **1372件パス**（48ファイル。内訳：コア 498件＋Electron 874件） |
...
| **確認画面：カメラ切替**（差し替え・時間調整・追加/削除・XML再出力） | ✅ 実装済み・実機確認済み。⚠️ **カメラ修正だけが FCP7 XML を書き換える**（重なり・尺超過が残ると再出力できない） |
| **確認画面：マーカー**（名前・コメントの修正・削除・XML再出力） | ✅ 実装済み・実機確認済み。⚠️ 追加・時刻/種別の変更は不可。⚠️ **CHECK系は再解析で修正が外れる**（画面で個別警告） |
| 孤立修正の再接続UI | ⏳ 未着手 |
...
| 7 | **確認画面：カメラ切替**（差し替え・時間調整・追加/削除・FCP7 XML 再出力） | ✅ 完了 |
| 8 | **確認画面：マーカー**（名前・コメントの修正・削除・FCP7 XML 再出力） | ✅ 完了 |
| 9 | **共通化のリファクタリング** — ★次に着手（4画面の共通項が確定） | ⏳ 未着手 |
| 10 | 孤立修正の再接続UI | ⏳ 未着手 |
| 11 | 書き出し画面 | ⏳ 未着手 |
| 12 | AI設定（ローカルモード配線 → GeminiProvider） | ⏳ 未着手 |

★**カメラ切替（Step 7）とマーカー（Step 8）が FCP7 XML を書き換えます。** 字幕はSRT、ショート候補はCSVにしか出ませんが、この2つはPremiereプロジェクトそのもの（映像トラック構成・マーカー）を変えます。Premiere実機検証を行う際は、これらを修正した状態でも確認してください。

4画面（字幕・ショート・カメラ・マーカー）が出そろい共通項が確定しました。Step 9 では共通化を別フェーズ・別コミットとして実施します。詳細は CURRENT-STATE.md の「10. 今後のリファクタリング候補」を参照してください。

> 実装の進捗・詳細・未検証事項は [CURRENT-STATE.md](./CURRENT-STATE.md) が唯一の正です。
> ⚠️ 本ファイルの「Phase 1/2/3」と「Step 1〜12」は**別の区分**です。
